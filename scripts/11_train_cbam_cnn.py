# -*- coding: utf-8 -*-
"""
scripts/11_train_cbam_cnn.py
============================
Train Model C: CBAM-CNN Model.

Applies Convolutional Block Attention Module (CBAM) attention enhancement
to the conventional CNN baseline encoder and decoder.
...
from src.models import CBAMCNN, MaskedMSELoss
...
def main():
    print("=" * 70)
    print("STAGE 4 — TRAINING MODEL C: CBAM-CNN (ATTENTION-ENHANCED CNN)")
    print("=" * 70)
    ...
    checkpoint_path = models_dir / "cbam_cnn_best.pt"
    history_path = metrics_dir / "cbam_cnn_history.json"
    ...
    model = CBAMCNN(in_channels=7, num_depths=15).to(device)
    param_count = count_parameters(model)
    print(f"CBAM-CNN Trainable Parameters: {param_count:,}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"Epoch [{epoch:02d}/{num_epochs:02d}] "
              ...
              f"Time: {t_epoch:.2f}s", flush=True)
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(f"  [SAVED CHECKPOINT] New best val loss: {best_val_loss:.6f}", flush=True)
        else:
            patience_counter += 1
    ...
    print(f"\nCBAM-CNN Training Complete! Total time: {total_training_time:.2f}s | Best Val Loss: {best_val_loss:.6f}")
